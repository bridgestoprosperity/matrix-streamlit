...
class defaultCostsSuspendedBridge:
    # A class to store the default costs from the excel sheet of data
    def __init__(self, path):
        self.linear_m_costs = pd.read_excel(path, sheet_name='Per linear m costs_suspended', header=0, index_col=0)
        self.lump_sum_costs = pd.read_excel(path, sheet_name='Lumpsum costs_suspended', header=0, index_col=0)
...
class defaultCostsSuspensionBridge:
    # A class to store the default costs from the excel sheet of data for the suspension bridge
    def __init__(self, path):
        self.linear_m_costs = pd.read_excel(path, sheet_name='Per linear m costs_suspension', header=0, index_col=0)
        self.lump_sum_costs = pd.read_excel(path, sheet_name='Lumpsum costs_suspension', header=0, index_col=0)
        ...
        # Store the specific lump sum costs as attributes of the class
        self.concrete_works_suspension_bridge = self.lump_sum_costs.loc['Concrete Works', 'Lumpsum cost (USD)']
        self.steel_reinf_suspension_bridge = self.lump_sum_costs.loc['Steel Reinforcement', 'Lumpsum cost (USD)']
        self.tower_system = self.lump_sum_costs.loc['Tower System', 'Lumpsum cost (USD)']


class defaultUnitCosts:
    # A class to store the default costs from the excel sheet of data
    def __init__(self, path):
